...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Premium streetwear essentials sourced for high-volume retail. Uncompromising materials, structured cuts, earth-toned palettes." },
      { property: "og:title", content: "Tomstill - Your Streetwear Wholesaler From India" },
      { property: "og:description", content: "Premium streetwear essentials sourced for high-volume retail. Uncompromising materials, structured cuts, earth-toned palettes." },
    ],
  }),
    ...
    <main className="flex-grow w-full">
      <section className="w-full flex flex-col lg:flex-row border-b border-outline h-auto lg:h-[716px] min-h-[500px]">
        <div className="w-full lg:w-1/2 p-4 md:p-8 flex flex-col justify-center border-r-0 lg:border-r border-outline bg-surface">
          <h1 className="font-display text-5xl md:text-7xl text-primary leading-tight uppercase max-w-2xl mb-8">
            Industrial<br />Grade<br />Quality
          </h1>
      ...
      <section className="w-full px-4 md:px-8 py-24 bg-surface">
        <div className="flex justify-between items-end mb-12 border-b border-outline pb-4">
          <h2 className="font-display text-3xl text-primary uppercase tracking-tighter">New Arrivals</h2>
          <Link
            to="/categories"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border-t border-l border-outline">
          {newArrivals.map((prod) => (
            <div key={prod.sku} className="border-b border-r border-outline p-4 group hover:bg-surface-container-low transition-colors duration-300 flex flex-col">
              ...
              <div className="mt-auto border-t border-outline pt-4 flex flex-col gap-1">
                ...
                <p className="font-display text-outline uppercase tracking-wider mb-4">SKU: {prod.sku}</p>
                <button
                  onClick={() =>
                    openWhatsApp(
      ...
      <section className="w-full relative py-32 border-y border-outline overflow-hidden bg-surface-variant">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center px-4">
          <h2 className="font-display text-4xl text-primary uppercase leading-none mb-6">Bulk Logistics</h2>
          <p className="font-body text-lg text-on-surface-variant max-w-2xl mx-auto mb-10">
            Streamlined international shipping. Factory-direct sourcing. We handle the complexity of large-scale garment fulfillment so you can focus on retail.
          </p>
          <div className="flex flex-col md:flex-row justify-center gap-8">
            <div className="text-center">
              <span className="block font-display text-3xl text-secondary">48h</span>
              <span className="block font-display text-xs uppercase tracking-widest text-outline">Dispatch</span>
            </div>
            <div className="text-center md:border-l md:border-r border-outline px-8">
              ...
              <span className="block font-display text-xs uppercase tracking-widest text-outline">Per Style</span>
            </div>
            <div className="text-center">
              <span className="block font-display text-3xl text-secondary">Global</span>
              <span className="block font-display text-xs uppercase tracking-widest text-outline">International Delivery</span>
            </div>
          </div>
        </div>
      </section>
