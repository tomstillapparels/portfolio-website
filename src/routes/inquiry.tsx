...
function InquiryPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const get = (id: string) =>
      (form.querySelector<HTMLInputElement | HTMLSelectElement>(`#${id}`)?.value ?? "").trim();
    openWhatsApp(
      [
      ...
      <main className="flex-grow flex items-center justify-center p-10">
        <div className="brutalist-border bg-surface-container-lowest p-10 text-center max-w-md">
          ...
          <p className="font-body text-on-surface-variant mb-8">
            Your manifest is pre-loaded in WhatsApp. Press send in the WhatsApp window to complete
            the inquiry — until then it has not reached our allocation desk. Once sent, expect a
            preliminary catalog transmission within 12 hours.
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
        <div className="lg:col-span-8 brutalist-border bg-surface-container-lowest p-6 md:p-10">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              className="brutalist-button w-full md:w-auto px-8 py-4 font-display uppercase text-xl mt-8 flex items-center justify-center gap-2"
              type="submit"
            >
              Transmit Inquiry
